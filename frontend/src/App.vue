      ...
      >
        ...
        <div class="tasks-section" v-if="todoTasks.length">
          ...
          <article class="task-detail" v-if="currentTask">
            ...
            <section
              v-if="currentTaskFailureDetail"
              class="failure-block"
            >
              <h3>失败原因</h3>
              <pre class="block-pre failure-pre">{{ currentTaskFailureDetail }}</pre>
            </section>
...
const currentTaskFailureDetail = computed(
  () => currentTask.value?.failureDetail ?? ""
);
